import { Injectable } from '@nestjs/common';
import { RoleFunction as PrismaRoleFunction } from '@prisma/client';
import { PrismaService } from 'src/integrations/prisma';
import { AbstractDeleteRoleFunctionRepository, RoleFunction } from '../../domain/repositories';

function toDomainEntity(prismaEntity: PrismaRoleFunction): RoleFunction {
  if (!prismaEntity) return null;
  return {
    id: prismaEntity.id,
    details: prismaEntity.details,
    notes: prismaEntity.notes,
    created_at: prismaEntity.created_at,
    updated_at: prismaEntity.updated_at,
  };
}

@Injectable()
export class PrismaDeleteRoleFunctionRepository implements AbstractDeleteRoleFunctionRepository {
  constructor(private readonly prisma: PrismaService) {}

  async execute(id: string): Promise<RoleFunction | null> {
    try {
      const prismaRoleFunction = await this.prisma.roleFunction.delete({
        where: { id },
      });
      return toDomainEntity(prismaRoleFunction);
    } catch (error) {
      if (error.code === 'P2025') { 
        return null;
      }
      throw error;
    }
  }
}
